...
| 32-to-1 selector path at the fast corner | not simulated | `docs/hardware_todo.md` item 2 | before final trust |
...
The selector path from oscillator to counter has not been simulated at the fast
corner, where the ring runs near 888 MHz. That is item 2 in the hardware list and
it is the last piece of the measurement chain I cannot vouch for. After it comes
item 8, the per-instance integration of Arm B, which decides whether the sixteen
macro copies really behave as one number.

Second, two results have no raw logs in the repository. The distributed-RC
comparison and the boundary flop sweep were both run in `/tmp` and only their
derived numbers survive, so a stranger with a clone can read them but cannot
recompute them. The decks are deterministic, so regenerating and archiving the
logs is mechanical work I owe.

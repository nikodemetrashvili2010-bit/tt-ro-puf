...
Stub ports carried `inout` until 2026-09-07, so the elaboration checked
names and arity and not direction. `extraction/cell_library.py` exists now
and derives direction from each cell's published view, so the stubs carry it
and iverilog checks it. A pin the library does not know, which means a
supply pin or one of ro_macro_hard's, is still `inout`.
...
def load_directions(path=CELL_LIBRARY_JSON):
    """Pin directions per base cell, from the derived library.

    Until 2026-09-07 every stub port was declared `inout`, with a note in
    the docstring saying direction belonged to extraction/cell_library.py
    when that module was split out. It is, so the stubs carry real
    directions and the elaboration checks them.

    A pin the library does not know stays `inout`, and there are two kinds:
    the supply pins, which are not signals, and ro_macro_hard, which is
    this project's own hardened macro and has no published functional view.
    Falling back rather than failing is deliberate. A stub is scaffolding
    for the elaboration and an unknown pin should not stop it; what would
    be wrong is silently calling a known output an input.
    """
    try:
    ...
    for name, cell in doc["cells"].items():
        ...
        for pin in cell["outputs"]:
            d[pin] = "output"
        for pin in cell["supplies"]:
            d[pin] = "inout"
        out[name] = d
    return out


def write_stubs(path, pins, directions=None):
    ...
    lines = ["// generated by lint_rtl.py from the pin names the design uses",
             "// directions from extraction/CELL_LIBRARY.json, inout where",
             "// the library does not know the cell",
             "`timescale 1ps/1ps", ""]
    for cell in sorted(pins):
        known = directions.get(base_master(cell) or "", {})
        ports = ", ".join("%s %s" % (known.get(p, "inout"), p)
                          for p in pins[cell])

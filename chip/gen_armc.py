...
def verilog_ring():
    """Arm C's ring. The same circuit as `dualarm/src/ro_macro.v` under its
    own name, so synthesis cannot share cells between the two arms.

    Node for node the same, which is not a thing to take on trust. The
    output buffer taps `n[N_INV/2]` because that is where ro_macro.v taps
    it; an earlier version of this function tapped `n[N_INV]`, the feedback
    node, and so hung the buffer load on a different node in each arm.
    Arm C exists to differ from Arm A in placement and in nothing else, and
    G.4 preregisters exactly that comparison, so a fixed circuit difference
    is a confound whichever way the number lands. `chip/lint_rtl.py` R08
    expands both generate loops and compares the two as netlists.
    """
...
// %d inverters and an output buffer tapped halfway along the chain. It
